...
def separate(inp=None, outp=None):
    ...
    if not files:
        print(f"No valid audio files in nigga")
        return
    print("Going to separate the files:")
    print('\n'.join(files))
    print("With command: ", " ".join(cmd))
    cmd.append(f'"{files[0]}"')
    print("Running command:", cmd) #OPTIONAL LINE
    finalcmd = ''
    for i in cmd:
       finalcmd += i
       finalcmd += ' '
...
def handle_upload(file, method):
    ...
    if method == 'mp3':
      fnamewpath = Path(file.name)
        ...
        fname += '.mp3'
        print("the final fucking filename is", fname)
        execution = f'yt-dlp -o "{fname}" --extract-audio --audio-format mp3 "{file}"'
        print("motherfucking the command line with this prompt:", execution)
        os.system(execution)
        #fnamewpath = Path('/BlendViz') / fname
    ...
    with open('blendscript.py', 'w') as f:
      f.write(code.format(*[flist[0], flist[1], flist[2], flist[3], colorlist, vary, option, voc, os.path.abspath(f'tmp_in/{fname}'), f'{os.path.abspath("download")}\\{fname[:-4]}.blend']))#  os.path.abspath(f'{fname[:-4]}.blend')]))

    #os.system(r'"C:\Program Files\Blender Foundation\Blender 3.5\blender.exe" --% -b template.blend --python blendscript.py')
    os.system(r'"C:\Program Files\Blender Foundation\Blender 3.5\blender.exe" -b template.blend --python blendscript.py')
